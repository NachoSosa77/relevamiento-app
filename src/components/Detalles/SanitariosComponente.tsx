...
export default function SanitariosComponent({
  localId,
  relevamientoId,
}: {
  ...
  const { data: equipamientos, loading } = useEquipamientoSanitario(
    localId,
    relevamientoId
  );

  if (loading)
    return <div className="text-sm text-gray-500">Cargando sanitarios...</div>;

  if (equipamientos.length === 0)
    return <div>No hay equipamientos sanitarios cargados.</div>;
