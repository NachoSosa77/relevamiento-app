...
export async function GET(
  ...
  try {
    ...
    const [rows] = await connection.query<RowDataPacket[]>(
      `
      SELECT id, condicion, disponibilidad, superficie_iluminacion, superficie_ventilacion, local_id, relevamiento_id
      FROM iluminacion_ventilacion
...
export async function PATCH(
  ...
  values.push(condicionId);

  try {
    const connection = await getConnection();
    const [result] = await connection.query(
      `UPDATE iluminacion_ventilacion SET ${fields.join(", ")} WHERE id = ?`,
      values
    );
    connection.release();
