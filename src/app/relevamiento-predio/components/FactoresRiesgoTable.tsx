import { Column, FactoresRiesgoAmbiental } from '@/interfaces/FactoresRiesgoAmbienta';
...
const FactoresRiesgoTable: React.FC<FactoresRiesgoFormProps> = ({ serviciosData, columnsConfig }) => {
  const [servicios, setServicios] = useState<FactoresRiesgoAmbiental[]>(serviciosData);
  const [formData, setFormData] = useState<FactoresRiesgoAmbiental[]>(serviciosData)

  const handleChange = (index: number, field: keyof FactoresRiesgoAmbiental, value: string) => {
