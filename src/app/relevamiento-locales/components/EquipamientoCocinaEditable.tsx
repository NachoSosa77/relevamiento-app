...
import { equipamientoCocina } from "../config/equipamientoCocina";
...
interface EquipamientoCocinaEditableProps {
  localId: number;
  relevamientoId: number;
}

const EquipamientoCocinaEditable = ({
  localId,
  relevamientoId,
}: EquipamientoCocinaEditableProps) => {
  const { data, loading, error, isSaving, updateData } =
    useLocalArray<Equipamiento>(localId, relevamientoId, "equipamientoCocina");

  const [responses, setResponses] = useState<Record<string, Equipamiento>>({});

  // Inicializa responses a partir de data
  useEffect(() => {
    if (data && data.length > 0) {
      const initial: Record<string, Equipamiento> = {};
      data.forEach((item) => {
  ...
  const handleCantidadChange = (
    question: string,
    field: "cantidad" | "cantidad_funcionamiento",
    value: number | undefined
  ) => {
    const current = responses[question];
    if (!current) return;
    const updated = { ...current, [field]: value ?? null };
    setResponses((prev) => ({ ...prev, [question]: updated }));
    updateData(updated);
  };

  const handleEstadoChange = (question: string, value: string) => {
    const current = responses[question];
    if (!current) return;
  ...
  return (
    <div className="mx-10 text-center p-4 rounded-xl shadow border border-gray-200 mb-4">
      <h2 className="font-semibold mb-2">Equipamiento Cocina/Offices</h2>
      <p className="text-xs text-gray-500 font-semibold mb-2">
        (Los cambios se guardan de manera automática)
      </p>

      <div className="flex flex-col gap-4">
        {equipamientoCocina.map(({ id, question, showCondition }) => {
          const current = responses[question];
          return (
            <div
              key={id}
            ...
            >
              ...
              <NumericInput
                ...
                onChange={(val) => handleCantidadChange(question, "cantidad", val)}
                label=""
                subLabel=""
                disabled={false}
              />
              ...
              <NumericInput
                ...
                onChange={(val) =>
                  handleCantidadChange(question, "cantidad_funcionamiento", val)
                }
                label=""
                subLabel=""
                disabled={false}
                  ...
                  {["Bueno", "Regular", "Malo"].map((estado) => (
                    <label key={estado} className="flex items-center gap-1">
                      <input
                        ...
                        onChange={() => handleEstadoChange(question, estado)}
                      />
                      {estado}
                    </label>
                  ))}
...
export default EquipamientoCocinaEditable;
